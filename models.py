...
class Membership(db.Model):
    ...
    max_poem_types = db.Column(db.Integer,
                               nullable=False)  # Number of poem types allowed
    max_frame_types = db.Column(
        db.Integer, nullable=False)  # Number of frame types allowed
    stripe_price_id= db.Column(db.String(255), unique=True, nullable=True)
    max_saved_poems = db.Column(
        db.Integer, nullable=False)  # Number of poems that can be saved
    has_gallery = db.Column(
        db.Boolean, default=False)  # Whether user has access to gallery
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
